...
- In order to perform the analysis we can the `nsys` tool. In the command line we execute the program via the `nsys`using `nsys profile -t nvtx,cuda -o results --stats=true --force-overwrite true ./a.out`.  
